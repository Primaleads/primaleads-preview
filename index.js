...
function renderRanking() {
  ...
  if (top5.length === 0) {
    rankingList.innerHTML = `<div class="empty">Noch keine Leads vorhanden.</div>`
    return
  }
...
function calculateStreetStats(streetName) {
  const streetLeads = cachedLeads.filter(lead => (lead.street || "") === streetName)

  const result = {
    street: streetName,
    total: streetLeads.length,
    pv: 0,
    wp: 0,
    no: 0,
    home: 0,
  ...
  streetLeads.forEach(lead => {
    if (lead.status === "PV Interesse") result.pv++
    if (lead.status === "WP Interesse") result.wp++
    if (lead.status === "Kein Interesse") result.no++
    if (lead.status === "Niemand zuhause") result.home++
  })

  result.score = (result.wp * 3) + (result.pv * 2) - (result.no * 1)
  return result
}

function renderTopStreets() {
  const streetMap = {}

  cachedLeads.forEach(lead => {
    if (!lead.street) return
    if (!streetMap[lead.street]) streetMap[lead.street] = true
  })

  const stats = Object.keys(streetMap)
    .map(street => calculateStreetStats(street))
  ...
  if (stats.length === 0) {
    container.innerHTML = `<div class="empty">Noch keine Straßenanalyse verfügbar.</div>`
    return
  }

  stats.forEach(item => {
    const el = document.createElement("div")
    el.className = "street-item"
    el.innerHTML = `
      <div class="street-name">${item.street}</div>
      <div class="street-meta">
        Gesamt: ${item.total} · PV: ${item.pv} · WP: ${item.wp} · Kein Interesse: ${item.no} · Niemand zuhause: ${item.home}
      </div>
      <div class="street-score">Score ${item.score}</div>
    `
...
function renderMyLeads() {
  ...
  if (myLeads.length === 0) {
    leadList.innerHTML = `<div class="card empty">Du hast noch keine Leads aufgenommen.</div>`
    return
  }
  ...
  map.on("click", async function(e) {
    ...
    try {
      ...
      if (data.address) {
        ...
        renderSelectedStreetSummary(street)
      }
    } catch (err) {
      console.log(err)
    }
...
function renderMapData() {
  ...
  if (selectedStreet) {
    renderSelectedStreetSummary(selectedStreet)
  }
}

function renderSelectedStreetSummary(street) {
  if (!street) {
    document.getElementById("streetSummary").innerText = "Klicke ein Haus auf der Karte an."
    return
  }

  const stats = calculateStreetStats(street)

  document.getElementById("streetSummary").innerHTML = `
    <strong>${street}</strong><br>
    Besucht: ${stats.total} · PV: ${stats.pv} · WP: ${stats.wp} · Kein Interesse: ${stats.no} · Niemand zuhause: ${stats.home}<br>
    Score: ${stats.score}
  `
}
